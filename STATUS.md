...
1. **"MCP servers" are not MCP servers.** ✅ **RESOLVED 2026-08-22** for `mcp_email.py`, `mcp_odoo.py`, `mcp_social.py` — they are now **real MCP protocol servers** (official `mcp` SDK 1.26.0: `Server` + `stdio_server` + `@list_tools`/`@call_tool`, JSON-RPC over stdio) via the shared `mcp_server_base.py` scaffold; each still keeps its argparse CLI as a fallback (run with no args or `--mcp` for MCP mode). Verified by `verify_mcp_servers.py`, a real MCP **client** that does the `initialize` handshake + `list_tools` + `call_tool` against each server as a subprocess (see "What Genuinely Works"). *(`mcp_browser.py` is not yet converted — still an argparse CLI.)* When originally written this item was accurate: none imported the MCP SDK, opened an `stdio_server`, or registered tools.
...
| 🥈 **Silver** | ⚠️ **PARTIAL** | Watchers + HITL + scheduling real; **"one working MCP server" now MET** (3 real MCP stdio servers, client-verified — see below); LinkedIn auto-post still broken. |
| 🥇 **Gold** | ❌ **NOT MET (as specified)** | Strong error-recovery; audit logging wired + verified (Gold #9); **email/odoo/social now exposed as real MCP servers (Gold #3/#6)**; but Odoo *transport* is XML-RPC not JSON-RPC and unproven against a live Odoo. |
...
- **Real MCP protocol servers (Silver #5, Gold #3/#6)** — `mcp_email.py`, `mcp_odoo.py`, `mcp_social.py` are now genuine **MCP servers** speaking JSON-RPC over stdio via the official `mcp` SDK (v1.26.0), not argparse CLIs. Shared scaffold `mcp_server_base.py` builds a low-level `Server`, registers `@server.list_tools()` + `@server.call_tool()` (`mcp_server_base.py:75`/`:79`), and serves over `stdio_server()` (`mcp_server_base.py:100`); sync logic methods run via `asyncio.to_thread`. Each file enters MCP mode when launched with no args or `--mcp` (`_MCP_MODE` guard at the top of each), and routes all logging to **stderr** first (`AI_EMPLOYEE_LOG_STREAM`, honored at `audit_logger.py:310`) so it never corrupts the stdout JSON-RPC channel; the argparse CLI is retained as an `else` fallback. Tools: email→`send_email`/`list_emails`/`draft_email`; odoo→`create_invoice`/`get_leads`/`update_lead`/`record_payment`; social→`post_social`/`draft_social`/`get_platform_status`. **Verified 2026-08-22** by `verify_mcp_servers.py` — a real MCP **client** that spawns each server as a subprocess and performs the `initialize` handshake + `list_tools` + a dry-run `call_tool` (exit 0, 3/3 PASS): handshakes returned `serverInfo.name` = `ai-employee-email`/`-odoo`/`-social`; tool lists matched exactly; `draft_email` produced `Drafts/email_20260822_025816.md`; `record_payment($5000, new payee)` was **refused by the security gate over the MCP boundary** (`requires_approval=True`, and the subprocess wrote a `mcp_odoo_record_payment status=requires_approval` row to `Logs/Audit/audit_20260822.jsonl`); `get_platform_status` returned JSON. All server logging appeared on stderr, proving stdout stayed protocol-clean. *(Scope: `mcp_browser.py` not yet converted; live Odoo/SMTP/Playwright round-trips remain **UNVERIFIED** — the MCP protocol layer + dry-run tool dispatch are what's proven.)*
...
- ~~**Any MCP-protocol server** — the `mcp_*.py` are CLIs, not MCP servers.~~ ✅ **RESOLVED 2026-08-22** — `mcp_email.py`, `mcp_odoo.py`, `mcp_social.py` are now real MCP stdio servers (client-verified; see "What Genuinely Works"). This unblocks **Silver #5, Gold #3, Gold #6**. *(Remaining CLI-only: `mcp_browser.py`.)*
